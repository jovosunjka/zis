...
<template match="*[attribute::property or attribute::rel or attribute::rev]">
   ...
   <if test="@class"> 
     <variable name="expended-class"><call-template name="expand-ns"><with-param name="qname" select="@class"/></call-template></variable>        
		 <element name = "rdfa:Description">
		   <attribute name="rdfa:about"><value-of select="$expanded-about" /></attribute>
		   <element name = "rdfa:type">
		     <attribute name="rdfa:resource"><value-of select="$expended-class" /></attribute>
		   </element>     
		 </element>
	 </if> -->
